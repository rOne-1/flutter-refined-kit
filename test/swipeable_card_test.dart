...
void main() {
  ...
  group('SwipeableCard', () {
    testWidgets('builder receives live drag state as the card is dragged',
        (tester) async {
    ...
    testWidgets('a small drag below both thresholds settles back, no commit',
        (tester) async {
      var committed = false;
      await tester.pumpWidget(harness(
        builder: (context, dragState) =>
    ...
    testWidgets(
        'a fast fling under the distance threshold still commits via velocity',
        (tester) async {
      String? direction;
      await tester.pumpWidget(harness(
        builder: (context, dragState) =>
    ...
    testWidgets(
        'onDirectionChanged reports the dominant axis direction, then null on release',
        (tester) async {
      ...
      final gesture = await tester
          .startGesture(tester.getCenter(find.byType(SwipeableCard)));
      await gesture
          .moveBy(const Offset(0, -50)); // up, past directionHintThreshold (30)
      await tester.pump();
    ...
    testWidgets(
        'onThresholdCrossed fires once per crossing, not once per pixel',
        (tester) async {
      var crossings = 0;
      await tester.pumpWidget(harness(
        builder: (context, dragState) =>
    ...
    testWidgets(
        'onCommitDecided fires immediately on release, before the fly-off finishes',
        (tester) async {
      final order = <String>[];
      await tester.pumpWidget(harness(
        builder: (context, dragState) =>
    ...
    testWidgets('external flyOff() via a GlobalKey commits programmatically',
        (tester) async {
      ...
      expect(
          direction, isNull); // this call's onComplete wasn't onSwipeCommitted
      expect(tester.takeException(), isNull);
    });

    testWidgets('entryDirection springs the card in from off-screen',
        (tester) async {
      SwipeDragState? latest;
      await tester.pumpWidget(harness(
        builder: (context, dragState) {
    ...
    testWidgets(
        'isInteractive: false renders the card with no gesture handling',
        (tester) async {
      var tapped = false;
      await tester.pumpWidget(harness(
        builder: (context, dragState) =>
    ...
    testWidgets('onTap and onLongPress pass through when interactive',
        (tester) async {
      var tapped = false;
