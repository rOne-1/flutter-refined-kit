...
/// chrome (top bar, search header, floating nav, ...) should collapse out
/// of view vs. be revealed again.
///
/// Ported as-is from The Lounge (`lib/utils/scroll_chrome_tracker.dart`) --
/// pure Dart/Flutter-core logic, no app-specific types.
///
...
/// [collapseThreshold] in one direction, for the caller to animate with
/// whatever transition it likes (a spring settle reads well here) -- rather
/// than a continuous per-pixel drag-follow or a stock `SliverAppBar` snap.
class ScrollChromeTracker {
  ...
  bool? handle(ScrollNotification notification) {
    ...
    // horizontal carousels bubbling their own ScrollNotifications up
    // through the same NotificationListener, which would otherwise
    // contaminate the collapse/expand signal with horizontal-swipe noise
    // unrelated to the user scrolling the page.
