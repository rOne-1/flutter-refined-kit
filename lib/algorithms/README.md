...
- **`weighted_rating.dart`** — the Bayesian weighted rating formula
  ...
  The Lounge's `lib/utils/weighted_rating.dart`. Also includes a new
  `meanRatingOf<T>` generic pool-mean helper (score-extractor-callback
  shape) — The Lounge's own `meanRatingOf`/`weightedRatingOf` took its own
  `MediaItem` type directly and didn't migrate; this is a fresh, actually-
  portable version of that same idea.
